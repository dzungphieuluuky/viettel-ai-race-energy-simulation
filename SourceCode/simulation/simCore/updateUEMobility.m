...
function handlers = getMobilityHandlers()
    ...
    handlers.highway_vehicle = @handleHighwayVehicleMobility;
end
...
function ue = handleHighwayVehicleMobility(ue, timeStep, currentTime)
    if ~isfield(ue, 'onHighway') || ~ue.onHighway
        ue = handleFastVehicleMobility(ue, timeStep, currentTime);
        return;
    end
    ...
    if isfield(ue, 'isForward') && ue.isForward
        ue.x = ue.x + distance; 
    else
        ue.x = ue.x - distance; 
    end
    
    if rand() < 0.05 && currentTime - ue.lastDirectionChange > 5
        if ue.isForward
            targetLane = ue.lane + (rand() < 0.5) * 2 - 1;  
            targetLane = max(1, min(ue.numLanes, targetLane));
            
            if targetLane ~= ue.lane
                ue.lane = targetLane;
                ue.y = (targetLane - 0.5) * ue.laneWidth;
                ue.lastDirectionChange = currentTime;
            end
        else
            targetLane = ue.lane + (rand() < 0.5) * 2 - 1;  
    ...
    if rand() < 0.1
        lateralShift = (rand() - 0.5) * 1.0;
        ue.y = ue.y + lateralShift;
        
        if ue.isForward
            targetY = (ue.lane - 0.5) * ue.laneWidth;
            ue.y = max(targetY - ue.laneWidth/2, min(targetY + ue.laneWidth/2, ue.y));
        else
            targetY = -(ue.lane - 0.5) * ue.laneWidth;
            ue.y = max(targetY - ue.laneWidth/2, min(targetY + ue.laneWidth/2, ue.y));
        end
    ...
    if ue.x > ue.highwayLength/2
        ue.x = -ue.highwayLength/2 + (ue.x - ue.highwayLength/2);
    elseif ue.x < -ue.highwayLength/2
        ue.x = ue.highwayLength/2 + (ue.x + ue.highwayLength/2);
    end
end
...
function handlers = getBoundaryHandlers()    
    ...
    handlers.highway = @enforceHighwayBounds;
...
function ue = enforceHighwayBounds(ue, simParams)
    if ~isfield(ue, 'onHighway') || ~ue.onHighway
        ue = enforceDefaultBounds(ue, simParams);
        return;
    end
    
    highwayLength = getFieldOrDefault(simParams, 'highwayLength', 10000);
    
    if ue.x > highwayLength/2
        ue.x = -highwayLength/2 + (ue.x - highwayLength/2);
    elseif ue.x < -highwayLength/2
        ue.x = highwayLength/2 + (ue.x + highwayLength/2);
    end
    
    numLanes = getFieldOrDefault(simParams, 'numLanes', 3);
    laneWidth = getFieldOrDefault(simParams, 'laneWidth', 3.5);
    totalWidth = numLanes * laneWidth;
    
    if isfield(ue, 'isForward') && ue.isForward
        if ue.y < 0
            ue.y = 0.5 * laneWidth;
        elseif ue.y > totalWidth
            ue.y = totalWidth - 0.5 * laneWidth;
        end
    else
        if ue.y > 0
            ue.y = -0.5 * laneWidth;
