...
function creators = getLayoutCreators()
    ...
    creators.highway = @createHighwayLayout;
end
...
function sites = createHighwayLayout(simParams, seed)
    numSites = simParams.numSites;
    highwayLength = simParams.highwayLength;
    
    sites = struct('id', {}, 'x', {}, 'y', {}, 'type', {});
    
    startPos = -highwayLength / 2;
    spacing = highwayLength / (numSites - 1);
    
    for i = 1:numSites
        xPos = startPos + (i - 1) * spacing;
        yOffset = 50 * (mod(i, 2) * 2 - 1); 
