...
function initializers = getUEInitializers()    
    ...
    initializers.highway = @initializeHighwayUEs;
...
function ues = initializeHighwayUEs(simParams, sites, seed)
    numUEs = simParams.numUEs;
    ...
    avgSpeed = simParams.ueSpeed / 3.6; 
    
    numForward = ceil(numUEs / 2);
    
    ues = [];
    for ueIdx = 1:numUEs
        x = -highwayLength/2 + rand() * highwayLength;
        
        if ueIdx <= numForward
            direction = 0; 
            laneIdx = randi(numLanes);
            y = (laneIdx - 0.5) * laneWidth; 
        else
            direction = pi; 
            laneIdx = randi(numLanes);
            y = -(laneIdx - 0.5) * laneWidth; 
        ...
        velocity = avgSpeed * (0.8 + 0.4 * rand());
        
        newUE = createUEStruct(ueIdx, x, y, ...
                              velocity, direction, ...
                              'highway_vehicle', simParams, seed);
        
        newUE.onHighway = true;
        newUE.highwayLength = highwayLength;
        newUE.lane = laneIdx;
        newUE.numLanes = numLanes;
        newUE.laneWidth = laneWidth;
        newUE.isForward = (ueIdx <= numForward);  
