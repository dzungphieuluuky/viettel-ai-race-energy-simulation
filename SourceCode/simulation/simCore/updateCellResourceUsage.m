function cells = updateCellResourceUsage(cells, ues)
    for cellIdx = 1:length(cells)
        ...
        cells(cellIdx).totalConnectionTime = cells(cellIdx).totalConnectionTime + numUEs;
        
        if cells(cellIdx).totalConnectionTime > 0
            actualDropRate = (cells(cellIdx).actualDropCount / cells(cellIdx).totalConnectionTime) * 100;
        else
            actualDropRate = 0;
        end
        ...
        if cells(cellIdx).txPower <= cells(cellIdx).minTxPower + (cells(cellIdx).maxTxPower - cells(cellIdx).minTxPower)/4
            powerDropPenalty = (cells(cellIdx).maxTxPower - (cells(cellIdx).minTxPower + (cells(cellIdx).maxTxPower - cells(cellIdx).minTxPower)/4) + 1) * 4.0;
        elseif cells(cellIdx).txPower <= cells(cellIdx).minTxPower + (cells(cellIdx).maxTxPower - cells(cellIdx).minTxPower)/2
            powerDropPenalty = (cells(cellIdx).maxTxPower - (cells(cellIdx).minTxPower + (cells(cellIdx).maxTxPower - cells(cellIdx).minTxPower)/2) + 1) * 1.5;
        else
            powerDropPenalty = 0;
        end
        ...
        theoreticalDropRate = min(25.0, baseDropRate + powerDropPenalty + congestionFactor + signalFactor);

        if cells(cellIdx).totalConnectionTime > 10  
            cells(cellIdx).dropRate = actualDropRate;
            cells(cellIdx).theoreticalDropRate = theoreticalDropRate;
        else
            cells(cellIdx).dropRate = theoreticalDropRate;
            cells(cellIdx).theoreticalDropRate = theoreticalDropRate;
        end
        
        if cells(cellIdx).totalConnectionTime > 1000
            cells(cellIdx).actualDropCount = floor(cells(cellIdx).actualDropCount * 0.9);
            cells(cellIdx).totalConnectionTime = floor(cells(cellIdx).totalConnectionTime * 0.9);
        end
        ...
        if cells(cellIdx).txPower <= cells(cellIdx).minTxPower + (cells(cellIdx).maxTxPower - cells(cellIdx).minTxPower)/4
            powerLatencyPenalty = (cells(cellIdx).maxTxPower - (cells(cellIdx).minTxPower + (cells(cellIdx).maxTxPower - cells(cellIdx).minTxPower)/4) + 1) * 15;
        elseif cells(cellIdx).txPower <= cells(cellIdx).minTxPower + (cells(cellIdx).maxTxPower - cells(cellIdx).minTxPower)/2
            powerLatencyPenalty = (cells(cellIdx).maxTxPower - (cells(cellIdx).minTxPower + (cells(cellIdx).maxTxPower - cells(cellIdx).minTxPower)/2) + 1) * 8;
        else
