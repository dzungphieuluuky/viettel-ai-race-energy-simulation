function simResults = runLoop(cells, ues, simParams, ESAgent, seed)
    ...
    for cellIdx = 1:length(cells)
        if ~isfield(cells(cellIdx), 'actualDropCount')
            cells(cellIdx).actualDropCount = 0;
        end
        if ~isfield(cells(cellIdx), 'totalDropEvents')
            cells(cellIdx).totalDropEvents = 0;
        end
        if ~isfield(cells(cellIdx), 'totalConnectionTime')
            cells(cellIdx).totalConnectionTime = 0;
        end
        if ~isfield(cells(cellIdx), 'theoreticalDropRate')
            cells(cellIdx).theoreticalDropRate = 0;
        end
    end
    ...
    for step = 1:numSteps
        ...
        [ues, cells] = updateUEDropEvents(ues, cells, simParams.ueLogFile, currentTime);
