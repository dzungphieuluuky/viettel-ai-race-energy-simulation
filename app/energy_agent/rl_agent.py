...
class StabilizedRewardCalculator:
    ...
    def update_weights(self, curriculum_phase: int):
        """Update weights based on curriculum phase."""
        self.curriculum_phase = curriculum_phase
        self.base_weights = self._get_base_weights(curriculum_phase)
        self._apply_rampup()

# In StabilizedRewardCalculator class

    def _get_base_weights(self, phase: int) -> Dict[str, float]:
        """Get base weights for a logical curriculum."""
        if phase == 0:  # Phase 0: Re-establish and maintain safety
            return {
                'drop_call': 400.0,
                'latency': 200.0,
                'cpu_violation': 100.0,
                'prb_violation': 100.0,
                'connection': 80.0,
                'energy': 1.0,        # Very small energy incentive, almost zero
                'perfect_bonus': 5.0
            }
        elif phase == 1:  # Phase 1: Introduce a balanced energy objective
            return {
                'drop_call': 400.0,      # Penalties remain high
                'latency': 200.0,
                'cpu_violation': 100.0,
                'prb_violation': 100.0,
                'connection': 80.0,
                'energy': 10.0,      # A moderate energy incentive
                'perfect_bonus': 8.0
            }
        else:  # Phase 2: Strong focus on energy with extreme safety enforcement
            return {
                'drop_call': 600.0,      # Penalties become even HIGHER
                'latency': 400.0,
                'cpu_violation': 200.0,
                'prb_violation': 200.0,
                'connection': 150.0,
                'energy': 25.0,      # A strong energy incentive
                'perfect_bonus': 10.0
            }
...
class RLAgent:
    def __init__(self, 
        ...
        self._load_latest_model() # This loads the agent AND the reward_calculator state
        
        # --- DELETED THE LINE THAT RESETS THE WEIGHTS ---
        # self.reward_calculator.update_weights(0) 
        
        # This line should come AFTER loading the model, to sync the state.
        self.curriculum_phase = self.reward_calculator.curriculum_phase 
        self.logger.info(f"Resuming with Curriculum Phase: {self.curriculum_phase}")
    ...
    def update(self, state: np.ndarray, action: np.ndarray, next_state: np.ndarray, done: bool):
        ...
        self.logger.info(f"Step {self.total_steps} | Reward: {reward:.10f} | Done: {done} | Buffer: {self.replay_buffer.pos}/{self.buffer_size}")
        ...
        new_phase = self.reward_calculator.curriculum_phase
        if self.total_steps >= 40000: # Adjust these step counts as needed
            new_phase = 2
        elif self.total_steps >= 25000:
            new_phase = 1
        
        if new_phase != self.reward_calculator.curriculum_phase:
            self.reward_calculator.update_weights(new_phase)
            self.curriculum_phase = new_phase
            self.logger.info(f"Advanced to Curriculum Phase: {self.curriculum_phase} at step {self.total_steps}")
    ...
    def _train(self):
        ...
        with torch.no_grad():
            self.log_alpha.data = torch.clamp(self.log_alpha.data, -2, 0.0)
        ...
        if self.total_steps % 100 == 0:
            self.logger.info(
                ...
                f"Q Mean: {(current_q1.mean().item() + current_q2.mean().item()) / 2:.2f} | \n"
                f"Q Mean in range [-10, 10]: {-10 <= (current_q1.mean().item() + current_q2.mean().item()) / 2 <= 10}"
            )
    ...
    def _load_latest_model(self):
        ...
        if not self.select_model:
            ...
            for file in model_files:
                if self.select_model in file:
                    latest_model_file = file
                    self.logger.info(f"Loading selected model: {latest_model_file}")
                    break
        try:
            ...
            self.reward_calculator = checkpoint.get('reward_calculator',
                                                   StabilizedRewardCalculator(curriculum_phase=self.curriculum_phase,
                                                                                total_steps=self.total_steps))
